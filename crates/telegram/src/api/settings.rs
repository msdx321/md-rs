...
use crate::config::{ChatConfig, Config, FILE, FileFormats};
...
async fn snapshot(cfg: Config, state: &ApiState) -> Result<Settings, Error> {
    let cursors = crate::storage::cursors::load(&state.database)
        .await
        .map_err(internal)?;
...
pub(super) async fn put(
    ...
    crate::storage::cursors::queue(&state.database, &updates)
        .await
        .map_err(internal)?;
