...
use crate::app::{AppCtx, TaskInfo, TaskState};
use crate::config::Config;
use crate::scheduler;
use crate::source::scraper::{self, VideoCard};
...
async fn list_videos(
    ...
    let cards = scraper::fetch_popular(&fetch, &cfg, page)
        .await
        .map_err(|e| ApiError::internal(crate::util::cloudflare_hint(&format!("{e:#}"))))?;
...
async fn start_download(
    ...
    tokio::spawn(async move {
        crate::downloader::download_video(ctx_for_task, card).await;
    });
...
async fn events(State(ctx): State<Arc<AppCtx>>) -> impl IntoResponse {
    ...
    media_runtime::web::events(futures_util::stream::select(tasks, statuses))
}
