...
use crate::source::browser::{BrowserMinter, BrowserOptions};
use crate::source::cf::{CookieSnapshot, CookieStore, MintFn};
use crate::source::http::{Fetcher, build_client};
use crate::storage::{Record, Repository, State};
use media_storage::Database;
...
async fn build_cookie_plumbing(
    ...
    if let Some(cookie) =
        crate::storage::cookie::load(database, &cfg.site_base, &cfg.cookie, &cfg.user_agent).await?
    {
    ...
    Arc::new(move |ua| {
        ...
        Box::pin(async move {
            ...
            if !minted.cookie.trim().is_empty() {
                let cookie = crate::storage::cookie::Cookie {
                    value: minted.cookie.clone(),
                    user_agent: minted.user_agent.clone(),
                };
                if let Err(error) = crate::storage::cookie::save(
                    &database,
                    &site,
                    &configured_cookie,
...
fn build_browser(cfg: &Config) -> Option<Arc<BrowserMinter>> {
    ...
    let browser_user_agent = if cfg.user_agent.trim().is_empty() {
        crate::source::cf::DEFAULT_USER_AGENT.to_string()
    } else {
        cfg.user_agent.clone()
    };
