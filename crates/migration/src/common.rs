...
pub(super) fn run() -> anyhow::Result<()> {
    let original: Value = if std::path::Path::new("config/app.yaml").exists() {
        media_config::load("config/app.yaml")?
    } else {
        Value::Null
    };
    let mut common = if original.is_null() {
        media_config::app::load_or_create()?
    } else {
    ...
    for (name, file) in [
        ...
        let old: Value = media_config::load(file)?;
    ...
    common.validate()?;
    media_config::app::FILE.save(&common)?;
    media_config::telegram::FILE.load_optional()?;
    media_config::jav::FILE.load_optional()?;
    Ok(())
}
