...
impl Repository {
    ...
    pub async fn forget_record(&self, id: &str) -> anyhow::Result<()> {
        let _write = self.state_write.lock().await;
        let tx = self.database.transaction().await?;
        tx.execute("DELETE FROM p91_records WHERE id=?", [id])
            .await?;
        tx.commit().await?;
    ...
    pub async fn mark_daily_run(&self, date: &str) -> anyhow::Result<()> {
        let _write = self.state_write.lock().await;
        self.database.connection().await.execute("INSERT INTO p91_scheduler(id,last_daily_run) VALUES (1,?) ON CONFLICT(id) DO UPDATE SET last_daily_run=excluded.last_daily_run", [date]).await?;
        self.state
            .lock()
            .expect("state lock poisoned")
...
impl State {
    pub async fn load(db: &Database) -> anyhow::Result<Self> {
        ...
        if let Some(row) = conn
            .query("SELECT last_daily_run FROM p91_scheduler WHERE id=1", ())
            .await?
            .next()
            .await?
        {
...
pub async fn store_record(conn: &Connection, record: &Record) -> anyhow::Result<()> {
    conn.execute("INSERT INTO p91_records(id,url,title,rank,status,path,size,finished_at,error) VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET url=excluded.url,title=excluded.title,rank=excluded.rank,status=excluded.status,path=excluded.path,size=excluded.size,finished_at=excluded.finished_at,error=excluded.error",
...
mod tests {
    ...
    fn record(id: &str, status: &str) -> Record {
        Record {
            id: id.into(),
            url: format!("https://www.91porn.com/view_video.php?viewkey={id}"),
    ...
    async fn round_trips_through_disk() {
        let dir = std::env::temp_dir().join(format!("p91d-state-{}", std::process::id()));
