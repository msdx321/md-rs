...
pub async fn paths(database: &crate::storage::Database) -> anyhow::Result<Vec<std::path::PathBuf>> {
...
pub async fn delete(temp_path: &Path, database: &crate::storage::Database) -> anyhow::Result<()> {
...
pub async fn load(conn: &crate::storage::Connection, key: &str) -> anyhow::Result<Option<u64>> {
    let mut rows = conn
        .query(
