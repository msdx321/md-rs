...
async function download(ds) {
  try {
    const r = await api('/p91/api/download', {
      method: 'POST',
      body: JSON.stringify({
        id: ds.id,
        url: ds.url,
...
function renderTasks() {
  ...
  nextRows.innerHTML = tasks.map((t) => {
    ...
    const pct = t.state === 'completed' ? 100
      : t.total_bytes > 0 ? Math.min(100, (t.downloaded_bytes / t.total_bytes) * 100)
      : 0;
...
async function taskAction(act, id) {
  ...
  const path = act === 'dismiss' ? `/p91/api/tasks/${id}` : `/p91/api/tasks/${id}/${act}`;
  try {
...
async function loadHistory() {
  try {
    const data = await api('/p91/api/history');
...
historyBody.addEventListener('click', (event) => {
  ...
  runAction(button, async () => {
    await api('/p91/api/history/' + encodeURIComponent(button.dataset.forget), { method: 'DELETE' });
...
$('btn-cancel').onclick = async () => { await api('/p91/api/daily/cancel', { method: 'POST' }); toast('Cancelling all downloads'); await loadTasks(); };
...
$('btn-clear-failed').onclick = () => runAction($('btn-clear-failed'), async () => {
  const result = await api('/p91/api/tasks/failed', { method: 'DELETE' });
...
$('btn-clear-history').onclick = async () => {
  ...
  await api('/p91/api/history', { method: 'DELETE' });
